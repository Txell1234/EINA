...
export default function ReputationDashboard() {
  ...
  const { data: report, isLoading } = useQuery({
    queryKey: ['reputation-report', selectedEntity],
    queryFn: () => reputationService.analyze(selectedEntity!, entityType),
    enabled: !!selectedEntity,
  })

  const { data: history } = useQuery({
    queryKey: ['reputation-history', selectedEntity],
    queryFn: () => reputationService.getHistory(selectedEntity!, 30),
    enabled: !!selectedEntity,
  })

  const { data: crisis } = useQuery({
    queryKey: ['reputation-crisis', selectedEntity],
    queryFn: () => reputationService.getCrisisIndicators(selectedEntity!),
    enabled: !!selectedEntity,
  })

  const handleAnalyze = () => {
    if (entityName.trim()) {
      setSelectedEntity(entityName.trim())
    }
  }
  ...
  return (
    <div className="reputation-dashboard">
      <h2>Dashboard de Reputación - {selectedEntity}</h2>
      ...
      {isLoading ? (
        ...
        <>
          <div className="reputation-score-card">
            <h3>Score de Reputación</h3>
            <div className="score-value">{report.reputation_score?.toFixed(1) || 'N/A'}</div>
            <div className={`trend ${report.sentiment_trend}`}>
              Tendencia: {report.sentiment_trend}
            </div>
          </div>

          {crisis && crisis.crisis_level !== 'none' && (
            <div className="crisis-alert">
              <h3>Indicadores de Crisis</h3>
              <div className="crisis-level">{crisis.crisis_level}</div>
              <ul>
                {crisis.crisis_indicators?.map((ind: any, idx: number) => (
                  <li key={idx}>{ind.description}</li>
                ))}
              </ul>
            </div>
          )}
          ...
          {history && (
            <div className="history-chart">
              <h3>Histórico</h3>
              <div className="trend-info">Tendencia: {history.trend}</div>
            </div>
          )}

          {report.stakeholder_sentiment && (
            <div className="stakeholder-analysis">
              <h3>Análisis por Stakeholder</h3>
              <div className="stakeholder-list">
                {Object.entries(report.stakeholder_sentiment).map(([type, data]: [string, any]) => (
                  <div key={type} className="stakeholder-item">
                    <strong>{type}</strong>: {data.average_sentiment?.toFixed(2) || 'N/A'}
                  </div>
                ))}
              </div>
            </div>
        ...
        <div>Error al cargar análisis</div>
      )}
    </div>
  )
}
