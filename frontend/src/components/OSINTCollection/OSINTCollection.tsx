...
const SOURCES: SourceConfig[] = [
  ...
  {
    id: 'ed_tiktok_hashtag',
    label: 'TikTok (hashtag)',
    category: 'xarxes_socials',
    description: 'Posts per hashtag TikTok. Requereix ENSEMBLEDATA_API_KEY.',
    requiresKey: true,
    fields: [
      { name: 'hashtag', label: 'Hashtag', placeholder: 'Ex.: geopolitics' },
      { name: 'count', label: 'Màx. posts (opcional)', placeholder: '30', type: 'number' },
    ],
    endpoint: 'ensembledata_tiktok_hashtag_posts',
    buildParams: (v, caseId) => ({
      hashtag: v.hashtag.replace(/^#/, ''),
      count: Math.min(parseInt(v.count || '30', 10) || 30, 100),
      case_id: caseId,
  ...
  },
  {
    ...
    category: 'xarxes_socials',
    description: 'Metadades d\'un post TikTok per URL. Requereix ENSEMBLEDATA_API_KEY.',
    requiresKey: true,
    fields: [{ name: 'post_url', label: 'URL del post', placeholder: 'https://www.tiktok.com/@user/video/...' }],
    endpoint: 'ensembledata_tiktok_post_info',
    buildParams: (v, caseId) => ({ post_url: v.post_url, case_id: caseId }),
  },
  {
    id: 'ed_instagram_user_info',
    label: 'Instagram (info usuari)',
    category: 'xarxes_socials',
    description: 'Perfil i metadades d\'un compte Instagram. Requereix ENSEMBLEDATA_API_KEY.',
    requiresKey: true,
    fields: [{ name: 'username', label: 'Usuari', placeholder: 'Ex.: reuters' }],
    endpoint: 'ensembledata_instagram_user_info',
    buildParams: (v, caseId) => ({
      username: v.username.replace(/^@/, ''),
      case_id: caseId,
    }),
  ...
  {
    id: 'ed_instagram_comments',
    label: 'Instagram (comentaris)',
    category: 'xarxes_socials',
    description: 'Comentaris d\'un post Instagram per URL. Requereix ENSEMBLEDATA_API_KEY.',
    requiresKey: true,
    fields: [
      { name: 'post_url', label: 'URL del post', placeholder: 'https://www.instagram.com/p/...' },
      { name: 'count', label: 'Màx. comentaris (opcional)', placeholder: '30', type: 'number' },
    ],
    endpoint: 'ensembledata_instagram_comments',
    buildParams: (v, caseId) => ({
      post_url: v.post_url,
      count: Math.min(parseInt(v.count || '30', 10) || 30, 100),
      case_id: caseId,
  ...
  {
    id: 'ed_twitter_user_info',
    label: 'Twitter/X (info usuari)',
    category: 'xarxes_socials',
    description: 'Perfil d\'un compte X. Requereix ENSEMBLEDATA_API_KEY.',
    requiresKey: true,
    fields: [{ name: 'username', label: 'Usuari', placeholder: 'Ex.: Reuters' }],
    endpoint: 'ensembledata_twitter_user_info',
    buildParams: (v, caseId) => ({
      username: v.username.replace(/^@/, ''),
      case_id: caseId,
    }),
  ...
  {
    id: 'ed_threads_user',
    label: 'Threads (usuari)',
    category: 'xarxes_socials',
    description: 'Timeline d\'un compte Threads. Requereix ENSEMBLEDATA_API_KEY.',
    requiresKey: true,
    fields: [
      { name: 'username', label: 'Usuari', placeholder: 'Ex.: meta' },
      { name: 'count', label: 'Màx. posts (opcional)', placeholder: '30', type: 'number' },
    ],
    endpoint: 'ensembledata_threads_user_posts',
    buildParams: (v, caseId) => ({
      username: v.username.replace(/^@/, ''),
      count: Math.min(parseInt(v.count || '30', 10) || 30, 100),
      case_id: caseId,
  ...
  {
    id: 'ed_youtube_channel',
    label: 'YouTube (canal)',
    category: 'xarxes_socials',
    description: 'Vídeos recents d\'un canal YouTube (channel ID). Requereix ENSEMBLEDATA_API_KEY.',
    requiresKey: true,
    fields: [
      { name: 'channel_id', label: 'Channel ID', placeholder: 'Ex.: UC...' },
      { name: 'count', label: 'Màx. vídeos (opcional)', placeholder: '30', type: 'number' },
    ],
    endpoint: 'ensembledata_youtube_channel_videos',
    buildParams: (v, caseId) => ({
