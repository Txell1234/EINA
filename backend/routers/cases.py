...
async def test_simple():
    """Endpoint de prueba ultra-simple - sin DB, sin validaciones"""
    logger.info("=== TEST SIMPLE ENDPOINT LLAMADO ===")
    return {"status": "ok", "message": "Backend responde correctamente"}

@router.post("/test-instant")
async def test_instant(prompt: str = Body(...)):
    """Endpoint de prueba instantáneo - responde inmediatamente sin DB"""
    import time
    start_time = time.time()
    logger.info(f"=== TEST INSTANT - Prompt recibido: {prompt[:50]} ===")
    
    # Responder inmediatamente sin hacer nada
    elapsed = time.time() - start_time
    logger.info(f"TEST INSTANT respondido en {elapsed:.3f}s")
    
    return {
        "status": "ok",
        "message": "Endpoint responde instantáneamente",
        "prompt_received": prompt[:100],
        "time_elapsed": f"{elapsed:.3f}s"
    }

@router.post("/test-create-fast")
async def test_create_fast(prompt: str = Body(...)):
    """Endpoint de prueba rápido - crea caso sin validaciones complejas"""
    import time
    start_time = time.time()
    logger.info(f"=== TEST CREATE FAST - Prompt: {prompt[:50]} ===")
    ...
    try:
        async with AsyncSessionLocal() as db:
            logger.info(f"DB session creada en {time.time() - start_time:.3f}s")
            
            new_case = Case(
                name=prompt[:50] if prompt else "Test",
                case_type="general",
                description=prompt or "Test case",
                status="pending",
                user_id=1,
            )
            db.add(new_case)
            logger.info(f"Caso agregado en {time.time() - start_time:.3f}s")
            
            await db.flush()
            case_id = new_case.id
            logger.info(f"Flush completado, ID: {case_id} en {time.time() - start_time:.3f}s")
            
            await db.commit()
            logger.info(f"Commit completado en {time.time() - start_time:.3f}s")
            
            await db.refresh(new_case)
            total_time = time.time() - start_time
            logger.info(f"Refresh completado. Tiempo total: {total_time:.3f}s")
            
            return {
                "status": "ok",
                "case_id": case_id,
                "message": "Caso creado rápidamente",
                "time_elapsed": f"{total_time:.3f}s",
                "case": CaseResponse.model_validate(new_case).model_dump()
            }
    except Exception as e:
        total_time = time.time() - start_time
        logger.error(f"Error en test-create-fast después de {total_time:.3f}s: {e}", exc_info=True)
        return {
            "status": "error",
            "error": str(e),
            "time_elapsed": f"{total_time:.3f}s",
            "message": "Error creando caso de prueba"
        }

@router.get("/test-connection")
async def test_connection():
    """Test endpoint para verificar conectividad con backend y base de datos"""
    try:
        from app.database import AsyncSessionLocal
        ...
        async with AsyncSessionLocal() as db:
            ...
            # Test adicional con query simple
            ...
            # Llistar primers 5 casos
            cases_result = await db.execute(
                select(Case).order_by(Case.created_at.desc()).limit(5)
            )
            sample_cases = cases_result.scalars().all()
            cases_info = [
                {
                    "id": c.id,
                    "name": c.name,
        ...
        return {
            ...
            "sample_cases": cases_info
        }
    except Exception as e:
        logger.error(f"Error en test-connection: {e}", exc_info=True)
        raise HTTPException(
...
async def test_create_simple(prompt: str = Body(...)):
    """Endpoint de prueba para crear caso - mínimo código"""
    return {
        "id": 1,
        "name": prompt[:50],
        "status": "pending",
